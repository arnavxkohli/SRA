...
class TabuGraph(Graph):
    ...
    def schedule_jobs(self, list_length: int, max_iterations: int, tolerance: int) -> None:
        tabu_list = deque(maxlen=list_length)
        best_schedule = self.schedule.copy()
        best_tardiness = self.calculate_tardiness_sum(best_schedule)
        ...
        for iteration in range(max_iterations):
            ...
            for i, j in interchanges:
                if self.is_valid_swap(current_schedule, i, j):
                    new_schedule = current_schedule.copy()
                    new_schedule[i], new_schedule[j] = new_schedule[j], new_schedule[i]
                    new_tardiness = self.calculate_tardiness_sum(new_schedule)
            ...
            if current_tardiness < best_tardiness:
                print(f"New best tardiness: {current_tardiness} at iteration {iteration + 1}, with schedule: {[s+1 for s in current_schedule]}")
